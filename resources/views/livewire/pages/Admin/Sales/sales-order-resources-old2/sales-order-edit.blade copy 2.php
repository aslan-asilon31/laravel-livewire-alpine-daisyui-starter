    ...
    <x-tabs wire:model="selectedTab" class="h-screen">
      <x-tab name="sales-order-tab" label="Sales Order" icon="o-users" class="h-screen">
        ...
        <x-form wire:submit="update" wire:confirm="Are you sure?">
          ...
          <x-select label="Customer" wire:model="masterForm.first_name" :options="$this->customers()" option-label="name"
            option-value="id" icon="o-user" />

          <x-datepicker label="Date" wire:model="masterForm.date" icon="o-calendar" hint="" />
