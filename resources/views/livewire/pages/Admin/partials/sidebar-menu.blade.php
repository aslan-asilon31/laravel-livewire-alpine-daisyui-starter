<x-menu activate-by-route>
  ...
  <x-menu-item title="Category Recommendation" icon="o-squares-2x2" link="/category-recommendations"
    :class="request()->is('category-recommendations') ? 'active' : ''" />


  <x-menu-separator title="Sales" icon="o-sparkles" />
  <x-menu-item title="Customer" icon="o-squares-2x2" link="/customers" wire:current="font-bold text-zinc-800" />
  <x-menu-item title="Sales Order " badge="{{ $salesOrderStatusPending->count() }} pending"
    badge-classes="!badge-warning" icon="o-squares-2x2" link="/sales-orders" :class="request()->is('products') ? 'active' : ''" />
